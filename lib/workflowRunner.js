...
function run( execution, res, req ){
...
function createDocs(execution, res, req){
...
function postAction( data ){
	return new Promise(function(resolve, reject){
		...
		if( thisElement.status === 'done' ){
			...
			var nextElement = execution.details[ thisElement['targetRef'] ];

			if( nextElement != undefined ){
				nextElement.submitResults = null;
			}
			...
			var details = execution.details[elementId];
			
			if( thisElement.type === 'manualTask' ){
				details = { name: thisElement.name };
			}
