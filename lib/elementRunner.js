...
function manualTask(elementId, execution){
	return new Promise(function(resolve){
		console.log("DO manualTask");

		var thisElement = execution.handlers[elementId];

		if( thisElement['status'] === 'wait' ){
			thisElement['status'] = 'done';
		}
		else{
...
function serviceTask(elementId, execution){
	...
	return new Promise(function(resolve){
		...
		Service.findOne({'_id': thisElement.service._id }, function(err, serviceResult){


			console.log( serviceResult );
			console.log( serviceResult.inputs );
...
function userTask(elementId, execution){
	...
	return new Promise(function(resolve){
		...
		if( execution.details[elementId].submitResults != null ){
			console.log("XXXX");
...
function exclusiveGateway(elementId, execution){
	...
	return new Promise(function(resolve){
		...
		for( var i = 0; i < wfVariables.length; i++ ){
			var text = 'var ' + wfVariables[i].name + ' = "' + wfVariables[i].value + '";';
			console.log( text )
			eval( text );
		}
		...
		for( var i = 0; i < conditionList.length; i++ ){
			console.log(  conditionList[i].content  );
		...
		for( var i = 0; i < parameterList.length; i++ ){
			...
			for( var j = 0; j < wfVariables.length; j++ ){
				...
				if( parameterList[i].first === wfVariables[j].name ){
					if( submitResults[parameterList[i].second] != null ){
						wfVariables[j].value = submitResults[parameterList[i].second];
					} 
					// in case parameter is an attach file.
					else if( submitResults['files'][parameterList[i].second] != null ){
						var filePath = submitResults['files'][parameterList[i].second].path;
						var filePathSplit = String( filePath ).split('/');
						var trueFilePath = filePathSplit[ filePathSplit.length - 1 ];

						wfVariables[j].value = '/uploads/document/' + trueFilePath;
					}
				}
			}
		}
