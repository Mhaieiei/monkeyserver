...
function magic( execution ){

	var currentElements = execution.currentElements;
	...
	while( currentElements.length > 0 ){
		...
		runElement( currentElements[0], execution );

		/*Array.prototype.push.apply( 
			currentElements, 
			doElement( currentElements.splice( 0, 1 )[0], handler )
		);*/
...
function runElement( element, execution ){

	var dummy = elementRunner[element.type];

	dummy( element, execution );

}

function doElement( element, handler ){



	if( handler[element]['bpmn2:outgoing'] != undefined ){
		return handler[element]['bpmn2:outgoing'];
	}
	else if( handler[element]['targetRef'] != undefined ){
		return [ handler[element]['targetRef'] ];
	}
