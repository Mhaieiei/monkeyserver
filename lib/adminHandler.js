        ...
        async.eachSeries(array,function(item,callback) {
          ...
          User.find( { 'local.name' :  item.local.username }, function (err, rows) {
            ...
            //if there is no user with that email
                ...
                newUser.local.password = newUser.generateHash(item.local.password);
                // save the user
...
router.post('/add_student_status',isLoggedIn,function(req,res){
      ...
      User.findOne( { 'local.username' :  req.query.user }, function (err, user) {
        ...
        var obj = {"status":req.body.status,"academicYear":req.body.academicYear,"careerOrHigherStudying":req.body.career};
                          ...
                          if(add_detail_student == 1){
                            ...
                            console.log('ADD detail TO student SUCCESSFUL : '+add_detail_student)
                              ...
                              User.update(
                                ...
                                detail:{$elemMatch:{academicYear:req.body.academicYear}}}, 
                                {$set: {'detail.$.status':req.body.status,'detail.$.careerOrHigherStudying':req.body.career}}
...
router.post('/add_role', isLoggedIn, function (req, res) {
  ...
  console.log('check-----role.id----------->'+role.id);
       ...
       res.redirect('/admin/view_user_role?user='+req.query.user+'&program='+req.query.program);
        ...
        dialog.info('Academic year has to be a number');
     }
...
router.get('/edit_specialTitle_admin',isLoggedIn,function(req,res){
    console.log("Admin edit_specialTitle_admin");
    ...
    // }
      ...
      Role.specialTitle.findOne({
        ...
        // console.log("role.jobDescription.length: "+role.jobDescription.length);
          ...
          res.render('admin/faculty/user/edit_specialTitle_admin.hbs', {
            layout: "adminPage",
                  role:role,
                  program: req.query.program   
                  // len:role.jobDescription.length
...
router.post('/edit_specialTitle_admin', isLoggedIn, function (req, res) {

      console.log("post edit_specialTitle_admin");
      // console.log("req.body.arrlen+ "+req.body.arrlen);
      console.log("req.query.program: "+req.query.program);
      ...
      res.redirect('/admin/view_specialTitle_admin?program='+req.query.program);
    }
      else{
...
router.get('/view_specialTitle_admin',isLoggedIn,function(req,res){
    console.log("Admin view_specialTitle_admin");
   ...
   Role.specialTitle.find({
    ...
    res.render('admin/faculty/user/view_specialTitle_admin.hbs', {
      layout: "adminPage",
            // user : req.query.user, // get the user out of session and pass to template
            program: req.query.program   ,
            docs:docs,
...
router.get('/view_specialTitle',isLoggedIn,function(req,res){
    console.log("Admin view_specialTitle");
   ...
   User.findOne(
          {"local.username":req.query.user})
         // , function(err, docs) {
      .populate('specialTitle')
      .exec(function (err, docs) {

        console.log("docs: "+docs);
 
    res.render('admin/faculty/user/view_specialTitle.hbs', {
      layout: "adminPage",
            user : req.query.user, // get the user out of session and pass to template
            program: req.query.program   ,
            docs:docs,
...
router.get('/del_specialTitle_admin',isLoggedIn,function(req,res){
    console.log("Admin del_specialTitle_admin");
   ...
   User.update(
            {}, 
            { $pull: { specialTitle: req.query.id} },
            {multi: true}
          , function(err, delete_role_user) {
            ...
            if (err){console.log('cant delete role from user'+err);}  
                  ...
                  // res.redirect('/aun/aun11-4?program='+req.query.program+'&year='+req.query.year+'&acid='+req.query.acid);
                  Role.specialTitle.remove({ '_id' : req.query.id },function(err, results) {
                  if (err){console.log('Delete Role.specialTitle err'+err);}
                  else{
...
router.get('/add_specialTitle',isLoggedIn,function(req,res){
    console.log("Admin add_specialTitle");
    ...
    res.render('admin/faculty/user/add_specialTitle.hbs', {
      layout: "adminPage",
            user : req.query.user, // get the user out of session and pass to template
            program: req.query.program   
        });
...
router.post('/add_specialTitle', isLoggedIn, function (req, res) {

      console.log("post add_specialTitle");
  ...
  console.log('check-----role.id----------->'+role.id);
         ...
         Role.specialTitle.findOne({
          ...
          console.log("EDIT-------req.query.program------------->:"+req.query.program);
            ...
            exist_role.type = exist_role.type;
            exist_role.title = req.body.title;
            exist_role.program = req.query.program;
            exist_role.academicYear = req.body.academicYear;
            exist_role.user.push(req.query.user);
            exist_role.role = req.body.role;
          ...
          exist_role.save(function (err) {
            ...
            User.update(
               { "local.username": req.query.user },
               { $push: { specialTitle: exist_role.id } }
            , function(err, add_eva) { 
        ...
        dialog.info('This user already has these title');
       }
          });
       }
       res.redirect('/admin/view_specialTitle?user='+req.query.user+'&program='+req.query.program);
     }
     else{
...
router.get('/del_specialTitle',isLoggedIn,function(req,res){
    console.log("Delete del_specialTitle");
    console.log(req.query.user);
    //console.log(req.query.email);
